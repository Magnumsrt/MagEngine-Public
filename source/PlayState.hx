...
class PlayState extends MusicBeatState
{
	...
	public var dadCameraOffset:Array<Float> = [0, 0];
	...
	override public function create()
	{
		...
		var folders:Array<String> = [Paths.modFolder('scripts'), Paths.getPreloadPath('scripts'), dumbShet];
		...
		if (dadCameraOffset != null)
			dadCameraOffset = stageData.camera_opponent;
	...
	public function moveCameraBySection(id:Int = 0)
	{
	...
	override public function update(elapsed:Float)
	{
		...
		if (camZooming)
		{
			lerpVal = CoolUtil.boundTo(1 - elapsed * 4.4, 0, 1);
	...
	private function popUpScore(note:Note):Void
	{
		...
		// ninjamuffin didn't optimized this function so i'm doing it myself bruh
		var coolPosition:FlxObject = new FlxObject(FlxG.width * 0.47, 0, 1, 1);
		coolPosition.screenCenter(Y);
		...
		if (combo >= 10)
		{
			...
			for (i in seperatedScore)
			{
				...
				numScore.x = coolPosition.x + (43 * daLoop) - 90;
				numScore.y += 80;
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || keyPressByController))
		{
			if (generatedMusic && !endingSong)
			{
					...
					if (daNote.canBeHit && daNote.mustPress && !daNote.tooLate && !daNote.wasGoodHit)
					{
						if (ignoreList.contains(daNote.noteData))
						{
							for (possibleNote in possibleNotes)
							{
								...
								{
									if (Math.abs(daNote.strumTime - possibleNote.strumTime) < 10)
				...
				// more accurate shit??? part 2 (back to the old times)
	...
	// for when the notes goes off-screen
	public function noteMiss(daNote:Note):Void
	{
		// remove too similar notes to be less cringe
		...
		callScripts('noteMiss', [daNote]);
	}

	// for hits in the void
	public function noteMissPress(direction:Int = 1):Void
	{
		if (!boyfriend.stunned)
		{
			...
			callScripts('noteMissPress', [direction]);
		}
	}
	...
	public function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (cpuControlled)
				strumPlayAnim(false, coolDat);
			else
			{
				var spr:StrumNote = playerStrums.members[coolDat];
				if (spr != null)
	...
	public function strumPlayAnim(isDad:Bool, id:Int)
	{
